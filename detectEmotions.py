...
import glob
from prompts import emotionPrompts
...
class DetectEmotion:
    ...
    def startProcess(self, images_path):
        ...
        emotion_prompts = emotionPrompts.emotion_prompt
        faces_path = []
        detected_emotions = []
        results = pd.DataFrame(columns=["FrameFileName", "FacePath", "Emotion"])
        try:
            list_of_images = []
            isFolder = True
            ...
            extract_faces_df.to_csv("Results/facesInfoEmotion.csv")
            ...
            for ind, row in extract_faces_df.iterrows():
                ...
                results.loc[df_length1] = [row['FrameFileName'], face_path, detected_emotion]
                if len(face_path) < 100:
                    faces_path.append(face_path)
                    detected_emotions.append(detected_emotion)
            collage = self.create_collage(faces_path, detected_emotions)

            # Save the collage
            collage_path = "Results/collageEmotion.jpg"
            collage.save(collage_path)
            collage.show()
            results.to_csv("Results/EmotionResult.csv")
        except Exception as e:
            print(e)
            raise
